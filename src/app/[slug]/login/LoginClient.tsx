  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f4f6f8', fontFamily: 'Poppins, sans-serif' }}>
      <div style={{ background: '#fff', borderRadius: 16, padding: 40, width: '100%', maxWidth: 380, boxShadow: '0 4px 24px rgba(0,0,0,0.08)' }}>
        ...
        {state?.error ? <div style={{ background: '#fef2f2', color: '#b91c1c', padding: '10px 14px', borderRadius: 8, marginBottom: 16, fontSize: 13 }}>{state.error}</div> : null}
        <form action={formAction} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <input type="hidden" name="slug" value={slug} />
          <input name="username" placeholder="Usuario" required style={inp} />
          <input name="password" type="password" placeholder="Senha" required style={inp} />
          <Btn />
        </form>
        <div style={{ textAlign: 'right', marginTop: 12 }}>
          <a href={`/${slug}/recuperar-senha`} style={{ color: '#17DB4E', fontSize: 13, fontWeight: 700, textDecoration: 'none' }}>
            Esqueci minha senha
          </a>
        </div>
        <div style={{ textAlign: 'center', marginTop: 16 }}>
          <a href={`/${slug}`} style={{ color: '#9ca3af', fontSize: 12, textDecoration: 'none' }}>
